...
const MarkAsNotDone: React.FC<Props> = observer(({style}) => {
  ...
  const markAllAsNotDone = () => {
    for (let i = 0; i < todoStore.data.get().length; i++) {
      todoStore.data.get()[i].isChecked = false;
    }
    todoStore.markedAllAsDone.set(false);
  };

  return (
    <TouchableOpacity style={style} onPress={markAllAsNotDone}>
